...
class EpipolarGeometry:
    ...
    def fundamental_matrix(self, view1_feat2D, view2_feat2D, optimize):
        ...
        if optimize == 1:
            ...
            F = np.asarray([np.transpose(f[0:3]), np.transpose(f[3:6]), [f[6], -(
                        -f[0] * f[4] + f[6] * f[2] * f[4] + f[3] * f[1] - f[6] * f[1] * f[5]) / (
                                                                                     -f[3] * f[2] + f[0] * f[5]), 1]]);
            F = F / sum(sum(F)) * 9;
    ...
    def polynomial_triangulation(self, feat1, feat2, epipole_1, epipole_2, F, P1, P2):
        ...
        p[1] = 2 * np.power(a, 4) + 2 * np.power(f2, 4) * np.power(c, 4) - 2 * np.power(f1, 4) * np.power(a,
                                                                                                          2) * np.power(
            d, 2) + 2 * np.power(f1, 4) * np.power(b, 2) * np.power(c, 2) + 4 * np.power(a, 2) * np.power(f2,
                                                                                                          2) * np.power(
            c, 2);
        p[2] = 8 * np.power(f2, 4) * d * np.power(c, 3) + 8 * b * np.power(a, 3) + 8 * b * a * np.power(f2,
                                                                                                        2) * np.power(c,
                                                                                                                      2) + 8 * np.power(
            f2, 2) * d * c * np.power(a, 2) - 4 * np.power(f1, 2) * np.power(a, 2) * c * d + 4 * np.power(f1,
                                                                                                          2) * a * np.power(
            c, 2) * b - 2 * np.power(f1, 4) * b * np.power(d, 2) * a + 2 * np.power(f1, 4) * np.power(b, 2) * d * c;
        p[3] = -4 * np.power(f1, 2) * np.power(a, 2) * np.power(d, 2) + 4 * np.power(f1, 2) * np.power(b, 2) * np.power(
            c, 2) + 4 * np.power(b, 2) * np.power(f2, 2) * np.power(c, 2) + 16 * b * a * np.power(f2,
                                                                                                  2) * d * c + 12 * np.power(
            f2, 4) * np.power(d, 2) * np.power(c, 2) + 12 * np.power(b, 2) * np.power(a, 2) + 4 * np.power(f2,
                                                                                                           2) * np.power(
            d, 2) * np.power(a, 2);
        p[4] = 8 * np.power(f2, 4) * np.power(d, 3) * c + 8 * np.power(b, 2) * np.power(f2, 2) * d * c + 8 * np.power(
            f2, 2) * np.power(d, 2) * b * a - 4 * np.power(f1, 2) * b * np.power(d, 2) * a + 4 * np.power(f1,
                                                                                                          2) * np.power(
            b, 2) * d * c + 2 * a * np.power(c, 2) * b + 8 * np.power(b, 3) * a - 2 * np.power(a, 2) * c * d;
        p[5] = 4 * np.power(b, 2) * np.power(f2, 2) * np.power(d, 2) + 2 * np.power(f2, 4) * np.power(d,
                                                                                                      4) + 2 * np.power(
            b, 2) * np.power(c, 2) - 2 * np.power(a, 2) * np.power(d, 2) + 2 * np.power(b, 4);
        p[6] = -2 * b * d * (a * d - b * c);
