...
class UncalibratedReconstruction:
    ...
    def estimate_initial_projection_matrices(self, H, epipole_2, p):
        ...
        P[:, :, 0] = [
            ...
            [0, 1, 0, 0],
            [0, 0, 1, 0]
        ]  # P1=[I | 0], i.e. frist frame aligned with world frame
    ...
    def get_initial_structure(self, feat_2D, P, epipole_1, epipole_2, F):
        ...
        for i in range(0, number_of_features):
            ...
            if self._triangulation_method == 0:
                ...
                x = self._eg_utils.polynomial_triangulation(
                    feat_2D[1, :, i], feat_2D[1, :, i], epipole_1, epipole_2, F, P[:, :, 0], P[:, :, 1]
                )
                x = x[0:3] / x[3]  # normalize

            if self._optimize_triangulation == 1:
                # refine 3D point estimation (due to noise, lines of sight may not intersect perfectly).
                # Minimizations should be carried out in the images
    ...
    def refine_3d_point(self, point3D, P1, P2, view1_feat2D, view2_feat2D):
        ...
        error = sum(
            self.compute_reprojection_error_point(
                P1, point3D, view1_feat2D
            ) + self.compute_reprojection_error_point(P2, point3D, view2_feat2D))
        # sdfds
        return error
    ...
    def self_calibration(self, P):
        ...
        for i in range(0, self._sequence_length - 1):
            ...
            AAA[0 + 4 * i, :] = [
                (
                        -np.power(P_tmp[1, 1], 2) + np.power(P_tmp[0, 1], 2) -
                        np.power(P_tmp[1, 0], 2) + np.power(P_tmp[0, 0], 2)
                ),
                (-2 * P_tmp[1, 0] * P_tmp[1, 3] + 2 * P_tmp[0, 0] * P_tmp[0, 3]),
            ...
            AAA[1 + 4 * i, :] = [
                (P_tmp[1, 0] * P_tmp[0, 0] + P_tmp[1, 1] * P_tmp[0, 1]),
                (P_tmp[1, 0] * P_tmp[0, 3] + P_tmp[1, 3] * P_tmp[0, 0]),
                (P_tmp[1, 1] * P_tmp[0, 3] + P_tmp[1, 3] * P_tmp[0, 1]),
                (P_tmp[1, 2] * P_tmp[0, 3] + P_tmp[1, 3] * P_tmp[0, 2]),
                P_tmp[1, 3] * P_tmp[0, 3],
                P_tmp[1, 2] * P_tmp[0, 2]
            ]

            AAA[2 + 4 * i, :] = [
                (P_tmp[2, 0] * P_tmp[0, 0] + P_tmp[2, 1] * P_tmp[0, 1]),
                (P_tmp[2, 0] * P_tmp[0, 3] + P_tmp[2, 3] * P_tmp[0, 0]),
                (P_tmp[2, 1] * P_tmp[0, 3] + P_tmp[2, 3] * P_tmp[0, 1]),
                (P_tmp[2, 2] * P_tmp[0, 3] + P_tmp[2, 3] * P_tmp[0, 2]),
                P_tmp[2, 3] * P_tmp[0, 3],
                P_tmp[2, 2] * P_tmp[0, 2]
            ]

            AAA[3 + 4 * i, :] = [
                (P_tmp[2, 0] * P_tmp[1, 0] + P_tmp[2, 1] * P_tmp[1, 1]),
                (P_tmp[2, 0] * P_tmp[1, 3] + P_tmp[2, 3] * P_tmp[1, 0]),
                (P_tmp[2, 1] * P_tmp[1, 3] + P_tmp[2, 3] * P_tmp[1, 1]),
                (P_tmp[2, 2] * P_tmp[1, 3] + P_tmp[2, 3] * P_tmp[1, 2]),
                P_tmp[2, 3] * P_tmp[1, 3],
                P_tmp[2, 2] * P_tmp[1, 2]
            ]
        ...
        if self._self_foc == 0:
            ...
            # optimize for varying focal lengths (to do)
...
def construct_3d_model(saved_model):
    print("--------------------------------")
    ...
    # Step 5: Optimize 3D points and projection matrices using the re-projection error
    ...
    # construct dynamic name based on ID
...
def get_images_feature_json(created_model: GeneratedFile):
    """
    generate combined image feature data
    :param created_model:
    :return:
    """
    submitted_images = created_model.model_images.all()
